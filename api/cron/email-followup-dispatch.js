...
export default async function handler(_request, response) {
  try {
    ...
    const cronSecret = normalizeString(process.env.EMAIL_FOLLOWUP_CRON_SECRET);

    if (!supabaseUrl || !cronSecret) {
      response.status(500).json({
        error: 'SUPABASE_URL and EMAIL_FOLLOWUP_CRON_SECRET are required.',
      });
      return;
    }

    const dispatchResponse = await fetch(`${supabaseUrl}/functions/v1/email-followup-dispatch`, {
      method: 'POST',
      headers: {
