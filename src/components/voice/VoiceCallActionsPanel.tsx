...
export function VoiceCallActionsPanel({
  ...
  return (
    <Card className="voice-call-section p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        <Button
          ...
          variant="secondary"
          size="sm"
          onClick={onRetryLeadCreate}
          disabled={!canRetryLead}
          loading={retryingLead}
          className="bg-white"
        >
          Retry lead create
        </Button>
        <Button
          ...
          variant="secondary"
          size="sm"
          onClick={() => onResolveReview('resolved')}
        ...
        <Button
          ...
          variant="ghost"
          size="sm"
          onClick={() => onResolveReview('dismissed')}
          loading={resolvingReview}
        >
          Dismiss
        </Button>
      ...
      <div className="mt-5 space-y-3">
        {actionRuns.length === 0 ? (
          <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm text-slate-600">
            No voice action runs have been queued for this call yet.
          </div>
        ) : (
          actionRuns.map((run) => (
            <div key={run.id} className="rounded-3xl border border-slate-200 bg-white p-4 shadow-[0_10px_20px_-22px_rgba(15,23,42,0.48)]">
              ...
              {run.last_error ? <div className="mt-2 text-xs text-rose-600">{run.last_error}</div> : null}
            </div>
          ))
        )}
      </div>
